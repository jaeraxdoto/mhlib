...
{
	mh_bare_solution *p=nullptr;	// pointer to solution
	int idx=0;	// index in population
	worstheap_item pqi;	// reference into worstheap
	pophashtable_elem() {}
	pophashtable_elem(mh_bare_solution *p1,int idx1, worstheap_item pqi1)
...
public:
	...
	void add(mh_bare_solution *p,int idx);
	/// Removes entry for this solution from the hash-table.
	void remove(mh_bare_solution *p);
